...
const api: ReaderBackend = {
  ...
  sports: {
    ...
    dotaStatus: () => request("sports.dota.status"),
    dotaYears: () => request("sports.dota.years.list"),
    dotaEvents: (params) => request("sports.dota.events.list", params),
    dotaEventMatches: (eventId) => request("sports.dota.event.matches", { eventId }),
    dotaMatchGet: (matchId) => request("sports.dota.match.get", { matchId }),
    dotaMatchWatch: (matchId) => request("sports.dota.match.watch", { matchId }),
    dotaMatchUnwatch: (matchId) => request("sports.dota.match.unwatch", { matchId }),
    dotaGameGet: (params) => request("sports.dota.game.get", params),
    dotaTeamGet: (teamId) => request("sports.dota.team.get", { teamId }),
    dotaTeamSearch: (query) => request("sports.dota.team.search", { query }),
    dotaTeamMatches: (params) => request("sports.dota.team.matches", params),
    dotaFollowedGet: () => request("sports.dota.followed.get"),
    dotaFollowedSet: (teamIds) => request("sports.dota.followed.set", { teamIds }),
    dotaFollowedToggle: (teamId) => request("sports.dota.followed.toggle", { teamId }),
    dotaPinnedGet: () => request("sports.dota.pinned.get"),
    dotaPinnedSet: (pins) => request("sports.dota.pinned.set", { pins }),
    dotaPinnedToggle: (eventId, eventType) =>
      request("sports.dota.pinned.toggle", { eventId, eventType: eventType ?? "tournament" }),
  },
  stories: {

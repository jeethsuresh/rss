...
func (s *Server) dispatch(ctx context.Context, req Request) (any, error) {
	...
	case "sports.dota.status":
		return s.svc.SportsDotaStatus(ctx)
	case "sports.dota.years.list":
		return s.svc.SportsDotaYears(ctx)
	case "sports.dota.events.list":
		var p struct {
			Year int `json:"year"`
		}
		_ = json.Unmarshal(req.Params, &p)
		return s.svc.SportsDotaEvents(ctx, p.Year)
	case "sports.dota.event.matches":
		var p struct {
			EventID string `json:"eventId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.EventID == "" {
			return nil, domain.ErrInvalidParams
		}
		return s.svc.SportsDotaEventMatches(ctx, p.EventID)
	case "sports.dota.match.get":
		var p struct {
			MatchID int `json:"matchId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.MatchID <= 0 {
			return nil, domain.ErrInvalidParams
		}
		return s.svc.SportsDotaMatchGet(ctx, p.MatchID)
	case "sports.dota.match.watch":
		var p struct {
			MatchID int `json:"matchId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.MatchID <= 0 {
		...
		return s.svc.SportsDotaMatchWatch(ctx, p.MatchID)
	case "sports.dota.match.unwatch":
		var p struct {
			MatchID int `json:"matchId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.MatchID <= 0 {
		...
		if err := s.svc.SportsDotaMatchUnwatch(ctx, p.MatchID); err != nil {
			return nil, err
		}
		return map[string]any{"ok": true}, nil
	case "sports.dota.game.get":
		var p struct {
			MatchID       int   `json:"matchId"`
			GameIndex     int   `json:"gameIndex"`
			StratzMatchID int64 `json:"stratzMatchId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.MatchID <= 0 {
			return nil, domain.ErrInvalidParams
		}
		return s.svc.SportsDotaGameGet(ctx, p.MatchID, p.GameIndex, p.StratzMatchID)
	case "sports.dota.team.get":
		var p struct {
			TeamID int `json:"teamId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.TeamID <= 0 {
		...
		return s.svc.SportsDotaTeamGet(ctx, p.TeamID)
	case "sports.dota.team.search":
		var p struct {
			Query string `json:"query"`
		}
		_ = json.Unmarshal(req.Params, &p)
		return s.svc.SportsDotaTeamSearch(ctx, p.Query)
	case "sports.dota.team.matches":
		var p struct {
			TeamID int `json:"teamId"`
			Year   int `json:"year"`
		}
		...
		return s.svc.SportsDotaTeamMatches(ctx, p.TeamID, p.Year)
	case "sports.dota.followed.get":
		return s.svc.SportsDotaFollowedGet(ctx)
	case "sports.dota.followed.set":
		var p struct {
			TeamIDs []int `json:"teamIds"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil {
		...
		return s.svc.SportsDotaFollowedSet(ctx, p.TeamIDs)
	case "sports.dota.followed.toggle":
		var p struct {
			TeamID int `json:"teamId"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil || p.TeamID <= 0 {
		...
		return s.svc.SportsDotaFollowedToggle(ctx, p.TeamID)
	case "sports.dota.pinned.get":
		return s.svc.SportsDotaPinnedGet(ctx)
	case "sports.dota.pinned.set":
		var p struct {
			Pins []domain.DotaPinnedEvent `json:"pins"`
		}
		if err := json.Unmarshal(req.Params, &p); err != nil {
			return nil, domain.ErrInvalidParams
		}
		return s.svc.SportsDotaPinnedSet(ctx, p.Pins)
	case "sports.dota.pinned.toggle":
		var p struct {
			EventID   string `json:"eventId"`
		...
		typ := domain.DotaEventType(p.EventType)
		if typ == "" {
			typ = domain.DotaEventTournament
		}
		return s.svc.SportsDotaPinnedToggle(ctx, p.EventID, typ)
